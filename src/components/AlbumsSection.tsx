...
const StackItem: React.FC<StackItemProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        <h3 className="text-xl md:text-3xl font-medium tracking-widest text-[#66a3db] drop-shadow-md">
          {album.title}
        </h3>
        <p className="text-sm md:text-base text-ocean-200/60 tracking-widest mt-1">
          {album.releaseYear}
        </p>
      </motion.div>
    </motion.div>
...
export function AlbumsSection() {
  ...
  const displayAlbums = ALBUMS.filter(
    (album) => !["a7", "a8", "a18"].includes(album.id),
  );
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      const next = Math.max(
        0,
        Math.min(displayAlbums.length - 1, current + delta),
      );

      // Capture scroll if we are within boundaries or moving into them
      if (
        (delta > 0 && current < displayAlbums.length - 1) ||
        (delta < 0 && current > 0)
      ) {
        e.preventDefault();
  ...
  const handlePlayTrack = (track: Track) => {
    ...
    if (
      album?.audioUrl &&
      (album.audioUrl.includes("y.qq.com") ||
        album.audioUrl.includes("c6.y.qq.com"))
    ) {
  ...
  const handlePlayAll = () => {
    ...
    if (firstAlbum && firstAlbum.tracks.length > 0) {
      if (
        firstAlbum.audioUrl &&
        (firstAlbum.audioUrl.includes("y.qq.com") ||
          firstAlbum.audioUrl.includes("c6.y.qq.com"))
      ) {
    ...
    <section ref={containerRef} className="relative w-full">
      ...
      <div className="relative w-full h-screen overflow-hidden">
        ...
        <div className="absolute top-12 md:top-24 left-8 md:left-24 z-20 pointer-events-none">
          ...
          >
            <h2 className="text-base md:text-lg font-medium tracking-[0.3em] text-ocean-400 uppercase mb-4">
              Latest Singles
            </h2>
            <h3 className="text-[54px] font-light text-neutral-900 tracking-tight mb-2">
              单曲
            </h3>
            <p className="text-xs md:text-sm font-light tracking-[0.2em] uppercase text-ocean-200/40">
        ...
        <div className="absolute top-12 md:top-24 right-8 md:right-24 z-20 flex items-center gap-4">
          ...
          <div className="relative flex items-center">
              ...
              {isSearchOpen && (
                ...
                >
                  <input
                    ...
                    className="w-full bg-white/50 backdrop-blur-md border border-ocean-900/10 rounded-full py-3 px-6 text-sm text-neutral-900 focus:outline-none focus:ring-2 focus:ring-ocean-500/20"
                  />
                        ...
                        {searchResults.map((album) => (
                          ...
                          >
                            ...
                            <div className="flex-1 overflow-hidden">
                              ...
                              <p className="text-xs text-ocean-900/50 uppercase tracking-widest truncate">
                                {album.artist}
                              </p>
                            </div>
                          </div>
            ...
            <button
              ...
              className="w-12 h-12 bg-white/50 hover:bg-white/80 backdrop-blur-md text-neutral-900 rounded-full flex items-center justify-center transition-colors shadow-sm border border-ocean-900/5"
            >
              {isSearchOpen ? (
                <X className="w-5 h-5" />
              ) : (
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 bg-white/50 hover:bg-white/80 backdrop-blur-md text-neutral-900 rounded-full transition-colors font-medium text-sm shadow-sm border border-ocean-900/5"
          >
          ...
          >
            ...
            <div className="flex items-center gap-4 w-full md:w-auto mt-2 md:mt-0 relative">
              ...
              <div>
                ...
                <p className="text-xs text-ocean-900/50 uppercase tracking-widest mt-0.5">
            ...
            <div className="flex items-center justify-between w-full md:w-auto gap-4 lg:gap-8 flex-1 md:justify-center">
              <div className="flex items-center gap-4 md:gap-6">
                <button
                  ...
                  className="text-ocean-900/50 hover:text-neutral-900 transition-opacity"
                ...
                <button
                  ...
                  className="w-10 h-10 md:w-12 md:h-12 bg-white hover:bg-ocean-100 text-neutral-900 rounded-full flex items-center justify-center shadow-[0_0_20px_rgba(255,255,255,0.2)] transition-colors"
              ...
              <div className="hidden md:flex items-center gap-3 w-48 lg:w-64">
                <span className="text-xs tracking-wider text-ocean-900/50 font-mono">
                  0:00
                </span>
                <div className="h-1 bg-ocean-100/50 rounded-full flex-1 overflow-hidden relative">
                  <motion.div
                    ...
                    transition={{
                      duration: isPlaying
                        ? parseInt(
                            (currentTrack?.duration || "0:00").replace(":", ""),
                          )
                        : 0,
                      ease: "linear",
                    }}
            ...
            <div className="absolute top-2 right-4 md:static md:w-auto flex items-center justify-end">
              <button
                ...
                className="w-8 h-8 flex items-center justify-center text-neutral-900/30 hover:text-neutral-900/80 transition-colors bg-white/5 rounded-full hover:bg-ocean-100/50"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
